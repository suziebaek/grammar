...
각 항목의 [AI 판별 신호(명시적 규칙)]을 반드시 적용하여 0점, 1점, 2점 기준에 맞게 문제를 출제하세요.

[항목별 점수 기준 및 명시적 출제 규칙]

A. 필요 문법 규칙 개수
   - [AI 판별 신호]: 정답 도출에 필요한 별개 문법 규칙을 모두 나열하고 개수를 카운트하여 설계합니다. (*주의: 같은 문법 범주의 하위 규칙은 1개로 카운트합니다. 예: 수동태의 형태(be+p.p.)와 수동태의 시제(was/were)는 '수동태' 1개로 카운트)
   - 0점: 1개
   - 1점: 2개
   - 2점: 3개 이상

B. 단서 위치
   - [AI 판별 신호]: 정답을 결정하는 단서(키워드/시제 신호어 등)가 문항 내 어디에 위치할지 의도적으로 배치합니다. (*단서란: 시제 신호어(now, yesterday, since, already 등), 접속사(when, before, after 등), 지시어(this, that, it 등), 문맥상 정답을 결정하는 키워드. 단서가 명시적 키워드가 아니라 의미적 맥락으로만 존재할 경우, 정답 결정에 필요한 정보가 담긴 가장 먼 절의 위치를 기준으로 판단합니다.)
   - 0점: 빈칸과 같은 절(clause) 내
   - 1점: 빈칸과 다른 절(인접 문장)
   - 2점: 문단 전체를 읽어야 단서 확인 가능

C. 오답 변별력
   - [AI 판별 신호]: 각 오답 선지가 "정답 규칙의 변형(시제/형태 오류)"인지 "전혀 무관한 규칙"인지 명확히 분류하여 선지를 구성합니다.
   - 0점: 오답 전부 무관한 규칙 (소거하기 쉬움)
   - 1점: 오답 중 정답과 같은 규칙의 변형이 1개 있음
   - 2점: 정답과 같은 규칙의 변형 오답이 2개 이상이거나, 정답보다 더 그럴듯해 보이는 오답이 1개 이상 있음. (*'그럴듯한 오답'의 판별 기준: 해당 오답을 고르는 학생의 흔한 오개념을 1줄로 설명할 수 있어야 합니다. 설명이 불가능하거나 철자, 시제 어미 등의 단순 형태 오류면 무관한 오답으로 간주합니다.)

D. 예외성
   - [AI 판별 신호]: 출제하는 규칙이 교재 내 "기본 규칙"인지, "예외/불규칙"인지 매칭하여 출제합니다. (목록에 없는 규칙은 임의로 예외로 분류하지 않고 기본 규칙으로 간주합니다.)
   - 0점: 기본 규칙 1개만 매칭
   - 1점: 기본 규칙 + 예외 규칙 1개 매칭
   - 2점: 예외 규칙만 매칭, 또는 예외의 예외
...
7. [소재의 다양성 강제]: 각 문항의 지문 소재(Topic)는 반드시 서로 완전히 다른 분야(예: 역사, 생물학, 예술, 경제, 심리학, 우주, 일상 에피소드 등)에서 골고루 출제하세요. 한 세트 내에서 동일하거나 유사한 소재가 2회 이상 반복되는 것을 엄격히 금지합니다.
{integration_rule}
=== 출력 형식 (반드시 준수) ===
【문제 N】
[발문]
